...
Computational workloads can be executed across a variety of CPU hardware architectures.

## Common Architecture Types

The two most commonly used architectures in modern computing environments are `linux/amd64` and `linux/arm64`. These architectures are based on distinct CPU instruction sets, each offering specific benefits and suited for different use cases:
...
## Wave's Multi-Architecture Support
