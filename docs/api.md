...
| `type`                              | This represents the type of package builder. Options: `CONDA` `CRAN`.                                                                                   |
...
| `commands`                          | Array of valid Dockerfile instructions to be included in the container (e.g., `RUN`, `CMD`, `ENTRYPOINT`, `USER`, `WORKDIR`, `ENV`, `COPY`, `ADD`). Each command must start with a valid Dockerfile instruction keyword.                                                                                                                       |
...
- Create Docker image with Pixi v1 template (multi-stage build):
