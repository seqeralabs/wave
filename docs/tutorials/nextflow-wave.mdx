...
description: Provision containers with Nextflow
...
This tutorial describes how to provision and freeze containers with Wave directly from your Nextflow pipelines. It includes:
...
1. In a terminal window, assign your access token to the `TOWER_ACCESS_TOKEN` environment variable:
    ...
    Replace `<access-token>` with the Seqera access token you created.
...
1. Create a `nextflow.config` file with the following contents:
    ...
    Replace `<repo-uri>` with your private container repository.
...
1. Create the `modules/gamma/main.nf` file for the module with the following contents:
    ...
    ```nextflow title="modules/gamma/main.nf"
    process HELLO {
      debug true
...
1. Create the `modules/gamma/Dockerfile` file for the module and add following contents:
    ...
    ```dockerfile title="modules/gamma/Dockerfile"
    FROM alpine
