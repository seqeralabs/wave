...
1. When a request is made to wave, first it increments the key with date. for example `builds/d/2024-04-23`.
2. Keys with organization are only incremented if the user is authenticated means there is seqera platform token in the request.
3. Wave extract the domain from the email id (For example: `test_metrics@seqera.io`) of the user, which it gets from seqera platform using the access token.
