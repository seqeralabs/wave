...
Security scanning identifies known vulnerabilities and outdated packages in container images. Wave uses [Trivy](https://trivy.dev/) to scan images. It can also produce a software bill of materials (SBOM) in SPDX format.
...
Wave also scans Nextflow plugin artifacts in OCI (Open Container Initiative) registries. It uses [ORAS](https://oras.land/) to retrieve the artifact and runs Trivy's filesystem scanner against it.
