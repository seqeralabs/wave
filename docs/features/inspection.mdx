...
date: 2026-04-22
tags: [inspection, manifest, metadata]
---

Container inspection returns a container image's metadata. The response includes the manifest, image config, layer digests, and platform-specific digest. A single request provides the information you need to decide whether to pull the image, trigger an augmentation, or record the image in an audit trail.
