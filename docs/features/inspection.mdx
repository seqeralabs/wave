...
Container inspection returns the metadata of a container image — the manifest, image config, layer digests, and platform-specific digest — without pulling the image layers. A single request provides the information required to decide whether to pull the image, trigger an augmentation, or record the image in an audit trail.
...
Inspection creates no tokens, pulls no layers, and does not touch the build or scan pipelines. Each request is independent.
