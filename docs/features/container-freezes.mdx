...
Freeze mode produces a permanent container image in a registry you control. Wave builds the image, pushes it to the target registry, and returns a stable URI that carries no Wave access token and does not expire. Subsequent pulls go directly to the registry and bypass Wave.
...
6. Wave responds with the frozen image URI, for example `your.registry.com/some/image/build:1a2b3c4d5e6f7890`. The tag is derived from the build hash and may include a recipe prefix for Conda builds. Singularity images use the `oras://` scheme.
...
:::note
When Nextflow is the Wave client, additional resources may be bundled as container layers — such as module resources and the workflow or module `bin/` directories. Changes to these bundled files affect the container fingerprint and trigger a rebuild. See [Bundling pipeline scripts](../nextflow/bundle-scripts.md) for details.
:::

Container freeze is not available in Wave Lite — see the [feature matrix](./index.mdx) for the capabilities supported by each deployment.
