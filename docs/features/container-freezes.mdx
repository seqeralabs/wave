...
Wave hashes the build inputs to produce a deterministic image ID. Inputs include the build file, package dependencies, target platform (AMD64 or ARM64), and target repository. Identical inputs return the existing image. With Nextflow, workflow `bin/` directories and module resources are also bundled, so changes trigger a rebuild (see [Bundling pipeline scripts](../nextflow/bundle-scripts.md)). Wave does not cache images locally unless you configure a cache repository.
