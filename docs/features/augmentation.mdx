...
Augmentation applies when a container request supplies a `containerConfig` with one or more layers. Wave returns an ephemeral image name, and when the container runtime pulls that image, it fetches the base layers directly from the source registry while Wave injects the additional layers.
...
5. The container runtime pulls base layer blobs from the source registry. Wave redirects these requests rather than serving the bytes itself. The runtime pulls injected layer blobs from Wave or from the source URL Wave returns, depending on how you supplied the layer.
