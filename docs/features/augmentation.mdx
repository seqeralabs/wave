...
1. A Wave client (Nextflow, the Wave CLI, or the Wave API) submits a container request with:
    ...
    - A `containerConfig` describing the layers and image-config overrides to inject. Supported overrides include environment variables, entrypoint, command, and working directory.
