...
Wave provides transparent access to private container registries. Credentials live in Seqera Platform, so you do not handle registry passwords, access tokens, or Docker config files directly.
...
3. Wave returns an ephemeral container image name, for example `wave.seqera.io/wt/<access-token>/library/alpine:latest`. The 12-character access token is a single-use key. It authorizes the follow-up pull without requiring the container runtime to supply source-registry credentials.
4. The container runtime pulls the ephemeral image. Wave resolves credentials for the source registry, fetches the manifest, and caches the image blobs. Wave serves blobs to the runtime through a Cloudflare CDN.
