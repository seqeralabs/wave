...
The IAM principal used by Wave needs the following permissions to obtain an ECR Public authorization token:

- `ecr-public:GetAuthorizationToken`
- `sts:GetServiceBearerToken` — required for ECR Public token exchange

For convenience, you can attach the AWS managed policy `AmazonElasticContainerRegistryPublicReadOnly` and grant `sts:GetServiceBearerToken` explicitly, or create a custom policy that includes only the permissions above.
