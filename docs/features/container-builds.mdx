...
- **Apptainer** resolves the architecture from the image index. Use `apptainer pull oras://<uri>` to get the image for the host architecture, or `apptainer pull --arch arm64 oras://<uri>` to select one explicitly.
