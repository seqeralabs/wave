...
The build flow involves the client, Wave, Seqera Platform, the build backend, the build registry, and the container runtime:
...
By default, Wave produces single-stage Conda images that bundle the package manager with the runtime environment. Two build templates run a two-stage build instead: the first stage installs the environment, and the second stage copies only the resolved environment into a minimal base image. Multi-stage builds typically reduce image size by 30–50%, exclude build tools from the final image, and emit a lock file for reproducibility.
