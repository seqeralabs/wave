...
class ContainerControllerTest extends Specification {
    ...
    def 'should throw BadRequestException when custom image name is not correct' () {
        ...
        def dockerAuth = Mock(ContainerInspectServiceImpl)
        def proxyRegistry = Mock(RegistryProxyService)
        def controller = new ContainerController(buildService: builder, dockerAuthService: dockerAuth, registryProxyService: proxyRegistry,
        ...
        then:
        def e = thrown(BadRequestException)
        e.message == "Provided image name is not valid - offending value 'foo bar'"
    }
