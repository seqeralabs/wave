...
class ContainerTokenControllerTest extends Specification {
    ...
    def 'should create build request' () {
        ...
        when:
        submit = new SubmitContainerTokenRequest(containerFile: encode('FROM foo'), spackFile: encode('some::spack-recipe'), spackArch: 'neoverse_v1', containerPlatform: 'arm64')
        build = controller.makeBuildRequest(submit, null, "")
        then:
        build.id == '058696cf7b89f4a3'
        ...
        build.spackArch == 'neoverse_v1'
        build.targetImage == 'wave/build:058696cf7b89f4a3'
        build.workDir == Path.of('/some/wsp').resolve(build.id)
        build.platform == ContainerPlatform.of('arm64')
    }
