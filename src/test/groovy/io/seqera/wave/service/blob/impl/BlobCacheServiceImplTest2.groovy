...
class BlobCacheServiceImplTest2 extends Specification implements AwsS3TestContainer {
    ...
    def 'handle job completion'() {
        ...
        def blob = BlobState.create('http://some/blob','s3://some/blob', [:], [:])
        ...
        then:
        1 * store.storeBlob(blob.getKey(), _ as BlobState) >> { id, BlobState info -> info.state==BlobState.State.ERRORED }

        when:
        service.onJobCompletion(job, blob, ok)
        then:
        1 * store.storeBlob(blob.getKey(), _ as BlobState) >> { id, BlobState info -> info.state==BlobState.State.COMPLETED }
