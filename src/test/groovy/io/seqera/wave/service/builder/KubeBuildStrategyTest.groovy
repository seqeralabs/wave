...
class KubeBuildStrategyTest extends Specification {
    ...
    def "request to build a container with right selector"(){
        ...
        resp
        and:
        1 * k8sService.buildContainer(_, _, _, _, _, [service:'wave-build']) >> null

        when:
        def req2 = new BuildRequest('from foo', PATH, repo, null, USER, ContainerPlatform.of('arm64'),'{}', cache, "")
        ...
        resp2
        and:
        1 * k8sService.buildContainer(_, _, _, _, _, [service:'wave-build-arm64']) >> null
