...
/**
 * Verify the 'wave.capabilities.credentials-federation' flag gates proxy-pull credential injection.
 *
 * @author Gavin Elder
 */
class RegistryProxyServiceCredentialsTest extends Specification {

    def 'should not inject credentials when federation disabled'() {
        given:
        def provider = Mock(RegistryCredentialsProvider)
        def service = new RegistryProxyService()
        service.@credentialsProvider = provider
        service.@credentialsFederation = false
        and:
    ...
    def 'should inject credentials when federation enabled'() {
        given:
        def provider = Mock(RegistryCredentialsProvider)
        def creds = Mock(RegistryCredentials)
        def service = new RegistryProxyService()
        service.@credentialsProvider = provider
        service.@credentialsFederation = true
        and:
        def route = Mock(RoutePath)
