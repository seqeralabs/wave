...
import io.seqera.wave.http.ProxyConfig
/**
...
class HttpClientConfig implements Retryable.Config {
     ...
     *
     * @return The resolved {@link HxProxyConfig} or {@code null} when no proxy is defined
     */
    private HxProxyConfig proxyConfig() {
        if( proxyUri )
            return ProxyConfig.resolve(proxyUri, proxyUsername, proxyPassword, proxyNoProxy)
        ...
        return ProxyConfig.fromEnvironment()
    }
     ...
     */
    static private void enableProxyAuthSchemes() {
