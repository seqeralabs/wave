...
class CredentialServiceImpl implements CredentialsService {
    ...
    ContainerRegistryKeys findRegistryCreds(String registryName, PlatformId identity) {
        ...
        final all = towerClient.listCredentials(identity.towerEndpoint, JwtAuth.from(identity), identity.workspaceId).get().credentials
        ...
        final encryptedCredentials = towerClient.fetchEncryptedCredentials(identity.towerEndpoint, JwtAuth.from(identity), creds.id, pairing.pairingId, identity.workspaceId).get()
