...
class RedisCounterProvider implements CounterProvider {
    ...
    Long get(String key, String field) {
        return redisService.hget(key, field) ? redisService.hget(key, field).toLong() : null
    }

    @Override
    Map<String, Long> getAllMatchingEntries(String key, String pattern) {
        final scanResult = redisService.hscan(key, "0", new ScanParams().match(pattern).count(hscanCount))
        if( !scanResult )
