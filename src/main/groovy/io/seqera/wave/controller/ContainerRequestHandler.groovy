...
/**
 * Shared handler that implements the core logic for container token submission,
 * record lookup, status lookup, and revocation. Both {@link ContainerController}
 * (alpha) and the upcoming {@code ContainersV1Controller} delegate to this bean.
 *
...
class ContainerRequestHandler {
    ...
    @Inject
    protected JwtAuthStore jwtAuthStore
    ...
    protected String towerEndpointUrl
    ...
    @Inject
    @Nullable
    protected ContainerBuildService buildService

    @Inject
    protected ContainerInspectService inspectService

    @Inject
    protected RegistryProxyService registryProxyService
    ...
    @Inject
    protected PairingChannel pairingChannel

    @Inject
    protected FreezeService freezeService

    @Inject
    protected ContainerInclusionService inclusionService
    ...
    @Inject
    @Nullable
    protected ContainerMirrorService mirrorService

    @Inject
    protected ContainerStatusService statusService

    @Inject
    @Nullable
    protected ContainerScanService scanService

    @Inject
    @Nullable
    protected MultiPlatformBuildService multiPlatformBuildService
    ...
    SubmitContainerTokenResponse submit(SubmitContainerTokenRequest req, HttpRequest httpRequest) {
        final resp = getContainerImpl(httpRequest, req, true)
        return resp.body()
    }

    WaveContainerRecord findRecord(String requestId) {
        return containerService.loadContainerRecord(requestId)
    }

    ContainerStatusResponse findStatus(String requestId) {
        return statusService.getContainerStatus(requestId)
    }
    ...
    // -------------------------------------------------------------------------
    // Internal implementation (verbatim from ContainerController)
    // -------------------------------------------------------------------------

    protected io.micronaut.http.HttpResponse<SubmitContainerTokenResponse> getContainerImpl(HttpRequest httpRequest, SubmitContainerTokenRequest req, boolean v2) {
    ...
    protected io.micronaut.http.HttpResponse<SubmitContainerTokenResponse> handleRequest(HttpRequest httpRequest, SubmitContainerTokenRequest req, PlatformId identity, boolean v2) {
        ...
        // return response
        return io.micronaut.http.HttpResponse.ok(resp)
    }
