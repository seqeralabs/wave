...
/**
 * HTTP Client Filter to prevent Server-Side Request Forgery (SSRF) attacks
 * by validating outgoing HTTP requests before they are executed.
 *
 * This filter intercepts all HTTP client requests and validates the target
 * host to ensure it doesn't point to internal/private resources.
 *
...
@Requires(property = 'wave.security.ssrf-protection.enabled', value = 'true', defaultValue = 'true')
class SsrfProtectionFilter {
    ...
    @RequestFilter
    void requestFilter(MutableHttpRequest<?> request) {
        def uri = request.getUri()
        log.trace "SSRF protection filter validating request to: ${uri}"

        try {
            validateUri(uri)
        } catch (IllegalArgumentException e) {
            log.warn "SSRF protection blocked request to: ${uri} - ${e.message}"
     ...
     */
    protected void validateIpAddress(InetAddress address) {
